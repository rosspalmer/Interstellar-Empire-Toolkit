...
import ross.palmer.interstellar.planet.map.voronoi.nodename.as3delaunay.LineSegment;
...
//        assignPolygonMoisture();
    ...
    public List<javafx.scene.shape.Polygon> getPolygons() {
        List<javafx.scene.shape.Polygon> polygons = new ArrayList<>();
        centers.forEach(center -> polygons.add(center.getPolygon()));
        return polygons;
    }
    ...
    private void buildGraph(Voronoi v) {
        ...
        final ArrayList<ross.palmer.interstellar.planet.map.voronoi.nodename.as3delaunay.Edge> libedges = v.edges();
        final HashMap<Integer, Corner> pointCornerMap = new HashMap();

        for (ross.palmer.interstellar.planet.map.voronoi.nodename.as3delaunay.Edge libedge : libedges) {
